<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            File # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            File
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <Number label="Size" v-model="value.size" :editMode="editMode" :inputUI="''"/>
            <Date label="CreatedTime" v-model="value.createdTime" :editMode="editMode" :inputUI="''"/>
